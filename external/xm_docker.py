...
def to_docker_executables(
  ...
  command_line = f'python -m my_process_entry --data_file={_DATA_FILE_NAME}'
  ...
  command_lst = [
                  # 'python -m pip install --upgrade jax==0.3.7 jaxlib==0.3.7+cuda11.cudnn82 -f https://storage.googleapis.com/jax-releases/jax_releases.html',
                  'python -m pip install "jax[cuda11_cudnn82]" -f https://storage.googleapis.com/jax-releases/jax_cuda_releases.html',
                  'python -m pip install --upgrade flax==0.4.1',
                   command_line
                   ]
  base_image = 'gcr.io/deeplearning-platform-release/tf-gpu.2-8'
  ...
  #         docker_instructions=[
  #             'ENV LD_LIBRARY_PATH=/lib:/usr/lib:/usr/local/lib:/usr/local/nvidia/lib64:/usr/local/cuda-11.0/targets/x86_64-linux/lib:/opt/conda/lib:/usr/local/cuda-11.0/targets/x86_64-linux/lib/stubs/',
  #             'RUN apt-get install -y git',
  #             f'RUN apt-get -y install libpython{python_version}',
  #             f'COPY {workdir_path}/requirements.txt requirements.txt',
  #             'RUN python -m pip install xmanager',
